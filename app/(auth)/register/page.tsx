...
import { signup, verifyPhone } from '@/features/auth/actions'
...
export default function RegisterPage() {
  ...
  const handleVerifyPhone = async (formData: FormData) => {
    setError(null)
    formData.append('phone', phone)
    const res = await verifyPhone(formData)
    if (res?.error) {
      setError(res.error)
    }
  }
      ...
      <div className="w-full max-w-md p-8 space-y-6 bg-white dark:bg-zinc-900 rounded-2xl shadow-xl border border-zinc-100 dark:border-zinc-800">
        <div className="space-y-2 text-center">
          ...
          <p className="text-zinc-500 dark:text-zinc-400">
            {step === 1 ? 'Enter your information to get started' : 'Enter the 6-digit code sent to your phone'}
          </p>
        </div>
        ...
        {step === 1 ? (
          ...
          <form action={handleVerifyPhone} className="space-y-4">
            <div className="space-y-2">
              <label className="text-sm font-medium leading-none" htmlFor="token">6-Digit Code</label>
              <input 
                id="token"
                name="token" 
                type="text" 
                required 
                maxLength={6}
                className="flex h-10 w-full rounded-md border border-zinc-200 dark:border-zinc-800 bg-transparent px-3 py-2 text-sm placeholder:text-zinc-500 focus:outline-none focus:ring-2 focus:ring-zinc-950 dark:focus:ring-zinc-300 text-center tracking-[0.5em] font-mono text-lg"
                placeholder="000000"
