...
export function MechanicRadar({ mechanics: initialMechanics }: MechanicRadarProps) {
        ...
        return (
          ...
          >
            <div className="relative group cursor-pointer">
              <motion.div 
                className="absolute inset-0 rounded-full bg-green-500/40 blur-sm"
                animate={{ scale: [1, 1.3, 1], opacity: [0.3, 0.6, 0.3] }}
                transition={{ duration: 2, repeat: Infinity, delay: Math.random() * 2 }}
              />
              <MapPin className="w-6 h-6 text-green-400 drop-shadow-[0_0_8px_rgba(74,222,128,0.8)] relative z-10" />
              ...
              <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 w-max opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none z-30">
                <div className="bg-zinc-800 text-white text-xs px-2 py-1 rounded shadow-lg border border-zinc-700 flex flex-col items-center">
                  <span className="font-semibold text-green-400">{mechanic.shop_name}</span>
                  <span className="text-zinc-400">{mechanic.distance.toFixed(1)} km away</span>
                </div>
              </div>
            </div>
          </motion.div>
